...
export async function POST(req: NextRequest) {
  try {
    ...
    const products = await prisma.product.findMany({
      ...
      select: { id: true, name: true, sku: true, unit: true, currentStock: true, purchasePrice: true, vatIncluded: true },
    });
    ...
    const matches = items.map((item: { description: string; quantity?: number; unitPrice?: number; total?: number }) => {
      ...
      return {
        ...
        matchedProduct: bestMatch
          ? { id: bestMatch.id, name: bestMatch.name, sku: bestMatch.sku, unit: bestMatch.unit, currentStock: bestMatch.currentStock, purchasePrice: bestMatch.purchasePrice, vatIncluded: bestMatch.vatIncluded, score: bestScore }
          : null,
      };
    });
