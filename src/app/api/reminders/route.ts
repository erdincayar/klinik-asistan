  ...
  try {
    ...
    switch (action) {
      case "send_reminder": {
        ...
        // Get patient name - verify belongs to clinic
      ...
      case "toggle_rule": {
        ...
        // Verify rule belongs to this clinic
        const existingRule = await prisma.reminder.findFirst({
          where: { id: ruleId, clinicId },
        });
        if (!existingRule) {
          return Response.json({ error: "Hatırlatıcı bulunamadı" }, { status: 404 });
        }
      ...
      case "delete_rule": {
        const { ruleId } = body;
        // Verify rule belongs to this clinic
        const ruleToDelete = await prisma.reminder.findFirst({
          where: { id: ruleId, clinicId },
        });
        if (!ruleToDelete) {
          return Response.json({ error: "Hatırlatıcı bulunamadı" }, { status: 404 });
