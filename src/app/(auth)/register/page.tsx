...
import { ArrowRight, ArrowLeft, Loader2, Eye, EyeOff, Check, X, Sparkles, LayoutDashboard, MessageCircle } from "lucide-react";
import { z } from "zod";
...
export default function RegisterPage() {
  ...
  async function handleStep1() {
    const valid = await trigger(["name", "email", "clinicName", "password", "confirmPassword"]);
    if (valid) {
      setStep(2);
    }
  }
    ...
    <>
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-gray-900">
          {step === 1 ? "Kayıt Ol" : "Deneme Sürenizi Başlatın"}
        </h1>
        <p className="mt-2 text-sm text-gray-500">
          {step === 1 ? "Poby hesabınızı oluşturun" : "7 gün boyunca ücretsiz deneyin"}
        </p>
        {/* Step indicators */}
        <div className="mt-4 flex gap-2">
          <div className={`h-1 flex-1 rounded-full ${step >= 1 ? "bg-[#1E1E2D]" : "bg-gray-200"}`} />
          <div className={`h-1 flex-1 rounded-full ${step >= 2 ? "bg-[#1E1E2D]" : "bg-gray-200"}`} />
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {error && (
          ...
          >
            {error}
            {googleError && (
              <Link href="/login" className="ml-1 font-semibold text-[#6366F1] hover:text-[#4F46E5] underline">
                Giriş Yap
              </Link>
            )}
          </motion.div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              ...
              {/* Clinic Name */}
              <div className="space-y-1.5">
                <label htmlFor="clinicName" className="text-sm font-medium text-gray-700">İşletme Adı</label>
                <input
                  id="clinicName"
                  type="text"
                    ...
                    {PASSWORD_RULES.map((rule) => {
                      ...
                      return (
                        <div key={rule.label} className="flex items-center gap-1.5">
                          {passed ? <Check className="h-3 w-3 text-emerald-500" /> : <X className="h-3 w-3 text-red-400" />}
                          <span className={`text-xs ${passed ? "text-emerald-600" : "text-gray-400"}`}>{rule.label}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <button
                type="button"
                onClick={handleStep1}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#1E1E2D] px-4 py-3 text-sm font-semibold text-white transition-all hover:bg-[#2A2A3C] focus:outline-none focus:ring-2 focus:ring-[#6366F1]/20"
              >
                Devam Et
          ...
          {step === 2 && (
            ...
            >
              {/* Trial info card */}
              <div className="rounded-xl border border-[#E0E7FF] bg-gradient-to-br from-[#EEF2FF] to-[#EEF2FF] p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#1E1E2D]">
                    <Sparkles className="h-5 w-5 text-white" />
                  </div>
                  <div>
                    <h3 className="text-base font-bold text-gray-900">7 Günlük Ücretsiz Deneme</h3>
                    <p className="text-xs text-gray-500">Deneme süresinde hiçbir ücret alınmaz</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500">
                    Deneme süresi sonunda planınıza istediğiniz modülleri ekleyebilirsiniz.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#1E1E2D] px-4 py-3 text-sm font-semibold text-white transition-all hover:bg-[#2A2A3C] focus:outline-none focus:ring-2 focus:ring-[#6366F1]/20 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {loading ? (
                    <>
      ...
      <p className="mt-6 text-center text-sm text-gray-500">
        ...
        <Link href="/login" className="font-semibold text-[#6366F1] hover:text-[#4F46E5]">
          Giriş Yap
        </Link>
      </p>
    </>
